...
// my func
void CAnimation::RemoveLastBitmap()
{
	bmp.pop_back();
	Reset();
}
void CAnimation::RemoveFirstBitmap()
{
	bmp.erase(bmp.begin());
	Reset();
}
int CAnimation::GetNumOfBitmap() 
{
	return bmp.size();
...
: NUM_GAME_STATES(7)
{
	...
	gameStateTable[GAME_STATE_RESET] = new CGameStateReset(this);
	gameStateTable[GAME_STATE_NEXT] = new CGameStateNextLevel(this);
	gameState = NULL;
}
